...
Oooof. Looking at that plot and the code we see now that the skill of the model is greatly decreased with R$^2$ = `r rsq` and RMSE = `r rmse`. We can also see that the predicted values are less than the observed values for these points.
